...
def test_hopfield_network_with_mnist():
    ...
    for i in range(num_of_tests):
        ...
        input_pattern = np.where(
            np.array(x_test[i]).reshape(784, 1) > 100, 1, -1)
        # store true label
        label = y_test[i]
    ...
    file = open("results_" + str(rule) + "_" +
                str(iteration_count) + ".json", "w+")
...
def display_confusion_matrix(data):
    ...
    sn.heatmap(df_cm, annot=True, cmap='gray_r', linecolor="black",
               linewidths=1, fmt='.0f', annot_kws={"size": 20})
...
def show_classification_progession():
    ...
    input_pattern = np.where(
        np.array(x_test[random_test_id]).reshape(784, 1) > 100, 1, -1)
    ...
    output_pattern_states, predictions = hn.recall_with_steps(
        input_pattern, steps_to_record, iteration_count)
...
def show_random_tests(test_count):
    ...
    random_test_samples = [random.randint(
        0, len(x_test) - 1) for _ in range(test_count)]

    # For every test sample
    for test_sample_index in random_test_samples:
        # reshape the mnist sample to match dimensions 784x1
        input_pattern = np.where(
            np.array(x_test[test_sample_index]).reshape(784, 1) > 100, 1, -1)
    ...
    for x in range(len(random_test_samples)):
        ...
        plt.subplot(2, len(random_test_samples),
                    x + len(random_test_samples) + 1)
        plt.axis('off')
...
# Classify a digit and display the input pattern and final state of the network
def classify(input_pattern):
    # Convert input_pattern to a numpy array
    ...
    plt.axis('off')
    plt.title("Input")
    plt.imshow(input_pattern.reshape((28, 28)), cmap='gray')

    # Plot output pattern and predicted label
    plt.subplot(2, 1, 2)
    plt.axis('off')
    plt.title("Prediction: " + str(label))
    plt.imshow(output_pattern.reshape((28, 28)), cmap='gray')
    plt.show()
